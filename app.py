...
if 'food_pool' not in st.session_state:
    FOOD_DATA = [
        ...
        {"name": "Том Ям Кха Гай", "img": "tomyam.jpg", "category": "Любитель гострого", "desc": "Кисло-гострий тайський том ям з куркою. На основі курячого бульйону з ароматним лемонграсом, імбиром та листям кафір-лайму та кокосовими вершками. Доповнений соковитим курячим філе, рибним соусом, печеним баклажаном, гливами, броколі, салатом ромен та кінзою."},
...
if st.session_state.index < len(st.session_state.food_pool):
    ...
    if not st.session_state.flipped:
        ...
        try:
            image_path = os.path.join(os.path.dirname(__file__), current_food["img"])
            ...
            st.warning(f"Помилка завантаження фото: {e}")
